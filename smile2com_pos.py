...
for task_id, smi in enumerate(list_smi):
    ...
    print(Chem.MolToMolBlock(m2),file=open(str(task_id)+"_pos.mol",'w+'))

    obconversion = openbabel.OBConversion()
    obconversion.SetInAndOutFormats("mol", "com")
    mol = openbabel.OBMol()

    obconversion.ReadFile(mol,str(task_id)+"_pos.mol")
    obconversion.WriteFile(mol,str(task_id)+"_pos.com")

    os.remove(str(task_id)+"_pos.mol")

    with open(str(task_id)+"_pos.com" ,'r+') as f_com:
        text = f_com.readlines()[5:]
    with open(str(task_id)+"_pos.com" ,'w+') as f_com:
        f_com.write("%mem=256MW\n# Opt Freq B3LYP/6-31+G* \
                     \n\n"+smi+"\n\n1 2\n")
    with open(str(task_id)+"_pos.com" ,'a+') as f_com:
        for line in text:
            f_com.write(line)
