...
data = pd.DataFrame({"smiles":list_smi})
...
for task_id, smi in enumerate(list_smi):
    ...
    print(Chem.MolToMolBlock(m2),file=open(str(task_id)+"_neu.mol",'w+'))

    obconversion = openbabel.OBConversion()
    obconversion.SetInAndOutFormats("mol", "com")
    mol = openbabel.OBMol()

    obconversion.ReadFile(mol,str(task_id)+"_neu.mol")
    obconversion.WriteFile(mol,str(task_id)+"_neu.com")

    os.remove(str(task_id)+"_neu.mol")

    with open(str(task_id)+"_neu.com" ,'r+') as f_com:
        text = f_com.readlines()[5:]
    with open(str(task_id)+"_neu.com" ,'w+') as f_com:
        f_com.write("%mem=256MW\n# Opt Freq B3LYP/6-31+G* \
                     \n\n"+smi+"\n\n0 1\n")
    with open(str(task_id)+"_neu.com" ,'a+') as f_com:
        for line in text:
            f_com.write(line)
